...
class AdminApp {
    ...
    bindEvents() {
        ...
        // 標記為已處理
        document.getElementById('markProcessed').addEventListener('click', async () => {
        ...
        document.getElementById('confirmEventDelete').addEventListener('click', async () => {
    ...
    async loadRegistrations() {
        try {
            // 必須從 Supabase 載入
            if (!window.supabaseClient || !window.supabaseClient.isAvailable()) {
                throw new Error('Supabase 不可用，無法載入報名資料');
            }
            
            const remoteRegistrations = await window.supabaseClient.getRemoteRegistrations();
            this.registrations = remoteRegistrations || [];
            
            // 更新本地快取
            localStorage.setItem('registrations', JSON.stringify(this.registrations));
            console.log('✅ 從 Supabase 載入報名資料');
            
        } catch (error) {
            console.error('❌ 載入報名資料失敗:', error);
            this.registrations = [];
            alert('載入報名資料失敗：' + error.message);
            // 不拋出錯誤，讓頁面繼續載入
        }
    ...
    renderListView() {
        ...
        tbody.innerHTML = this.filteredRegistrations.map(reg => `
            <tr>
                <td>${this.formatDate(reg.submitted_at)}</td>
                <td>${reg.name}</td>
                <td>
                    <div>${reg.email}</div>
    ...
    renderCardView() {
        ...
        container.innerHTML = this.filteredRegistrations.map(reg => `
            <div class="registration-card">
                ...
                <div class="card-meta">${this.formatDate(reg.submitted_at)}</div>
                <div class="card-body">
        ...
        return `
            ...
            <div class="detail-section">
                ...
                <div class="detail-field">
                    ...
                    <span class="detail-value">${this.formatDateTime(reg.submitted_at)}</span>
                </div>
                <div class="detail-field">
    ...
    async markAsProcessed() {
        ...
        try {
            if (!window.supabaseClient || !window.supabaseClient.isAvailable()) {
                throw new Error('Supabase 不可用，無法更新報名狀態');
            }
            
            await window.supabaseClient.updateRegistration(this.currentRegistration.id, {
                status: 'processed'
            });
            console.log('✅ 報名狀態已更新到 Supabase');
        } catch (error) {
            console.error('❌ 更新報名狀態到 Supabase 失敗:', error);
            throw error;
        }
    ...
    async deleteRegistration() {
        ...
        try {
            if (!window.supabaseClient || !window.supabaseClient.isAvailable()) {
                throw new Error('Supabase 不可用，無法刪除報名');
            }
            
            await window.supabaseClient.deleteRegistration(this.currentRegistration.id);
            console.log('✅ 報名資料已從 Supabase 刪除');
        } catch (error) {
            console.error('❌ 從 Supabase 刪除報名失敗:', error);
            throw error;
        }
    ...
    exportData() {
        ...
        const csvData = [
            ...
            ...this.registrations.map(reg => [
                this.formatDateTime(reg.submitted_at),
                reg.name,
                reg.email,
                reg.phone,
                reg.birthdate || '',
    ...
    async saveEvent() {
        ...
        try {
            if (!window.supabaseClient || !window.supabaseClient.isAvailable()) {
                throw new Error('Supabase 不可用，無法儲存活動');
            }
            
            if (this.currentEvent) {
                await window.supabaseClient.updateEvent(eventData.id, eventData);
                console.log('✅ 活動已更新到 Supabase');
            } else {
                await window.supabaseClient.createEvent(eventData);
                console.log('✅ 活動已新增到 Supabase');
            }
        } catch (error) {
            console.error('❌ 儲存活動到 Supabase 失敗:', error);
    ...
    async deleteEvent() {
        ...
        try {
            if (!window.supabaseClient || !window.supabaseClient.isAvailable()) {
                throw new Error('Supabase 不可用，無法刪除活動');
            }
            
            await window.supabaseClient.deleteEvent(this.currentEvent.id);
            console.log('✅ 活動已從 Supabase 刪除');
        } catch (error) {
            console.error('❌ 從 Supabase 刪除活動失敗:', error);
