...
-- 建立函數來檢查活動是否還有名額
CREATE OR REPLACE FUNCTION check_event_capacity(event_id TEXT)
RETURNS BOOLEAN AS $$
DECLARE
...
-- 建立函數來獲取活動的報名人數
CREATE OR REPLACE FUNCTION get_event_registration_count(event_id TEXT)
RETURNS INTEGER AS $$
DECLARE
...
-- 建立函數來檢查報名截止日期
CREATE OR REPLACE FUNCTION is_registration_open(event_id TEXT)
RETURNS BOOLEAN AS $$
DECLARE
    event_deadline DATE;
